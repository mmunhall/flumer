...
1. Reduce or eliminate completely the amount of log data stored on a filesystem,
2. Reduce or eliminate the amount of file IO on a filesystem,
3. Continue to allow logs to be written to the filesystem,
4. Store all log data in Hadoop for analysis,
5. Allow for old data to be purged,
6. Provide an interface to monitor log data in near real time,
7. Allow for the data to be analyzed programmatically (e.g. map/reduce, Spark, [insert buzzword])
